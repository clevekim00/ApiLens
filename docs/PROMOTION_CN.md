...
*   **Load Hub 性能管控**: 在多个远程代理上运行同一个工作流，并集中查看 RPS、错误率、延迟百分位、代理健康状态和机器资源。
...
### 5️⃣ Load Hub 分布式性能测试
在多台远程机器上运行基于工作流的负载测试，在 Load Hub 中实时汇总指标以及 CPU、内存、disk I/O、network 状态，并通过 drain、health check、rollback 流程管理远程代理升级。
...
4.  **扩展**：打开 Load Hub，将工作流分配给多个远程代理并导出最终报告。
