...
### 실시간 시각적 디버깅 (Visual Debugging)
ApiLens의 워크플로우는 실행 과정을 실시간으로 생중계합니다.
- **활성 노드**: 실행 중인 노드는 **파란색 글로우** 효과와 함께 펄스 애니메이션이 표시됩니다.
- **성공/실패 경로**: 데이터가 이동하는 경로(Edge)가 결과에 따라 **초록색(성공)** 또는 **빨간색(실패)**으로 하이라이트됩니다.
- **상태 애니메이션**: 응답 상태 코드(200 OK 등)가 노드 옆에 애니메이션으로 나타나 즉각적인 피드백을 제공합니다.

### 워크플로우 템플릿 (Templates)
전문적인 시나리오를 즉시 적용할 수 있는 템플릿 라이브러리를 제공합니다.
- **Auth & Data Flow**: 로그인을 통한 토큰 획득 및 데이터 동기화 예제.
- **GraphQL Explorer**: GraphQL 엔드포인트를 사용한 복잡한 쿼리 처리.
- **WebSocket Echo**: 실시간 양방향 통신 검증 예제.
- **Error Handling**: 조건 분기를 통한 지능형 에러 복구 시나리오.
...
- **검색**: 저장된 워크플로우, API 요청, 환경 변수 검색.
- **이동**: 특정 화면(Settings, Home 등)으로 즉시 점프.
- **명령**: 테마 전환, 데이터 Export, 모든 소켓 연결 끊기 등 빠른 실행.
