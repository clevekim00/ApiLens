...
class ExpressionEvaluator {
  ...
  static bool evaluate(String expression, Map<String, dynamic> context) {
    ...
    for (final candidate in operators) {
      index = resolved
          .indexOf(' $candidate '); // strict spacing for safety? or just index
      if (index != -1) {
        op = candidate;
        break;
      }
    ...
    if (op == null) {
      ...
      return trimmed == 'true' ||
          (trimmed != 'false' && trimmed != '0' && trimmed.isNotEmpty);
    }
    ...
    String right = parts
        .sublist(1)
        .join(' $op ')
        .trim(); // Join rest in case right side has same string? Unlikely with split.
    ...
    switch (op) {
      ...
      case '!=':
        return leftVal != rightVal;
      case '>':
        return (leftVal is num && rightVal is num) ? leftVal > rightVal : false;
      case '<':
        return (leftVal is num && rightVal is num) ? leftVal < rightVal : false;
      case '>=':
        return (leftVal is num && rightVal is num)
            ? leftVal >= rightVal
            : false;
      case '<=':
        return (leftVal is num && rightVal is num)
