...
class JsonDiffUtil {
  ...
  static List<DiffNode> _compareNodes(
      String key, dynamic oldVal, dynamic newVal) {
    // 1. If types are different, it's a change (unless both null?)
    if (oldVal.runtimeType != newVal.runtimeType &&
        oldVal != null &&
        newVal != null) {
    ...
    if (oldVal is Map && newVal is Map) {
      ...
      for (var k in allKeys) {
        if (!oldMap.containsKey(k)) {
          // Added
          nodes
              .add(DiffNode(key: k, newValue: newMap[k], type: DiffType.added));
        } else if (!newMap.containsKey(k)) {
          // Removed
          nodes.add(
              DiffNode(key: k, oldValue: oldMap[k], type: DiffType.removed));
        } else {
          // Both verify
          ...
          if (subDiffs.isNotEmpty) {
            ...
            final isPrimitiveChange = subDiffs.length == 1 &&
                subDiffs.first.key == k &&
                subDiffs.first.type == DiffType.changed;
            ...
            if (isPrimitiveChange) {
              ...
              final hasChanges =
                  subDiffs.any((n) => n.type != DiffType.unchanged);
              if (hasChanges) {
                nodes.add(DiffNode(
                    key: k,
                    type: DiffType.unchanged,
                    children:
                        subDiffs)); // Parent is unchanged container, children changed
              }
            }
          }
        }
    ...
    if (oldVal is List && newVal is List) {
      final List<DiffNode> nodes = [];
      final maxLen =
          oldVal.length > newVal.length ? oldVal.length : newVal.length;
      ...
      for (int i = 0; i < maxLen; i++) {
        ...
        if (i >= oldVal.length) {
          nodes
              .add(DiffNode(key: k, newValue: newVal[i], type: DiffType.added));
          listChanged = true;
        } else if (i >= newVal.length) {
          nodes.add(
              DiffNode(key: k, oldValue: oldVal[i], type: DiffType.removed));
          listChanged = true;
        } else {
          final subDiffs = _compareNodes(k, oldVal[i], newVal[i]);
          if (subDiffs.isNotEmpty) {
            nodes.addAll(
                subDiffs); // Flatten list items or wrap? Wrap is better really.
            listChanged = true;
          }
        }
      }
