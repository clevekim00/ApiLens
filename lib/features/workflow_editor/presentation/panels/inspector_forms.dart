      ...
      children: [
        DropdownButtonFormField<String>(
          ...
          items: const [
            ...
            DropdownMenuItem(
                value: 'configRef', child: Text('Use Saved Config')),
          ],
          onChanged: (val) {
            setState(() => _mode = val!);
            _save();
        ...
        if (_mode == 'direct')
          TextField(
            ...
            decoration: const InputDecoration(
                labelText: 'WebSocket URL', hintText: 'wss://..'),
            onChanged: (_) => _save(),
          )
        else
          Container(
            ...
            child: const Text(
                'Config Reference selection not implemented for MVP (using ws-config-001 stub)'),
          ),
        const SizedBox(height: 16),
        TextField(
          controller: _storeAsCtrl,
          decoration: const InputDecoration(
              labelText: 'Store Session As', hintText: 'e.g. mainWs'),
          onChanged: (_) => _save(),
        ),
      ],
    );
    ...
    return Column(
      children: [
        TextField(
          ...
          decoration: const InputDecoration(
              labelText: 'Session Key', hintText: 'mainWs'),
          onChanged: (_) => _save(),
        ),
        const SizedBox(height: 16),
        DropdownButtonFormField<String>(
        ...
        TextField(
          ...
          decoration: const InputDecoration(
              labelText: 'Payload (Template supported)',
...
class _WebSocketWaitFormState extends State<WebSocketWaitForm> {
  ...
  void initState() {
    ...
    _sessionKeyCtrl = TextEditingController(text: widget.config.sessionKey);
    _timeoutCtrl =
        TextEditingController(text: widget.config.timeoutMs.toString());
  ...
  void _save() {
    widget.onSave(WebSocketWaitNodeConfig(
      ...
      executionPolicy: widget.config.executionPolicy.copyWith(
        timeoutMs: int.tryParse(_timeoutCtrl.text) ?? 5000,
      ),
    ));
  }
          ...
          children: [
            Expanded(
              child: DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(
                      value: 'containsText', child: Text('Contains Text')),
                  DropdownMenuItem(
                      value: 'jsonPathEquals', child: Text('JSON Path Equals')),
                  DropdownMenuItem(
                      value: 'anyMessage', child: Text('Any Message')),
                ],
                onChanged: (val) {
        ...
        if (_matchType != 'anyMessage')
          TextField(
            ...
            decoration: InputDecoration(
              labelText: _matchType == 'containsText'
                  ? 'Text to contain'
                  : 'JSON Path (e.g. \$.type=="pong")',
            ),
            onChanged: (_) => _save(),
          ),
      ],
