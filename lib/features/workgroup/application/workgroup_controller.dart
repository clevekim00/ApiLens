...
class WorkgroupController extends StateNotifier<List<WorkgroupModel>> {
  ...
  Future<void> createGroup(String name, WorkgroupType type,
      {String? parentId, String? description}) async {
  ...
  Future<void> updateGroup(String id,
      {String? name, String? parentId, String? description}) async {
  ...
  Future<void> deleteGroup(String id, {bool moveToSystem = true}) async {
    final current = state.firstWhere((g) => g.id == id,
        orElse: () => WorkgroupModel.systemRoot());
    ...
    final requestController =
        _ref.read(savedRequestControllerProvider.notifier);
  ...
  String exportWorkgroup(String groupId) {
    ...
    final relatedRequests = allRequests
        .where((r) => r.groupId != null && groupIds.contains(r.groupId))
        .toList();
  ...
  Future<void> importWorkgroup(String jsonString,
      {String? targetParentId}) async {
    try {
      ...
      final rawGroups = (data['groups'] as List)
          .map((e) => WorkgroupModel.fromJson(e))
          .toList();
...
final workgroupControllerProvider =
    StateNotifierProvider<WorkgroupController, List<WorkgroupModel>>((ref) {
...
final folderChildrenProvider =
    Provider.family<List<WorkgroupModel>, String?>((ref, parentId) {
